...
export class AudioEngine {
  ...
  // Play sound in reverse using Web Audio API buffer source
  public async playSoundReverse(sound: Sound, velocity: number = 127): Promise<string> {
    if (!this.audioContext) {
      console.error('AudioContext not available for reverse playback');
      return '';
    }

    // For trigger mode, stop any already playing instances
    if (sound.settings.playMode === 'trigger' || sound.settings.playMode === 'loop') {
      const alreadyPlaying = Array.from(this.playingSounds.values()).filter(
        ps => ps.soundId === sound.id
      );
      if (alreadyPlaying.length > 0) {
        console.log(`🔄 Trigger mode: stopping already playing sound for reverse: ${sound.name}`);
        alreadyPlaying.forEach(ps => this.stopSound(ps.id));
      }
    }

    const playingId = `${sound.id}-${Date.now()}-reverse`;
    console.log(`⏪ Playing sound in reverse: ${sound.name}`);

    try {
      // Fetch and decode the audio file
      const response = await fetch(this.getFileUrl(sound.filePath));
      const arrayBuffer = await response.arrayBuffer();
      const audioBuffer = await this.audioContext.decodeAudioData(arrayBuffer);

      // Reverse the audio buffer
      const reversedBuffer = this.reverseAudioBuffer(audioBuffer, sound.settings.startTime, sound.settings.endTime);
      ...
      const velocityFactor = velocity / 127;
      const targetVolume = sound.settings.volume * velocityFactor * this.masterVolume;
      gainNode.gain.value = targetVolume;

      // Apply fade in
      if (sound.settings.fadeInMs > 0) {
        gainNode.gain.setValueAtTime(0, this.audioContext.currentTime);
        gainNode.gain.linearRampToValueAtTime(
          targetVolume,
          this.audioContext.currentTime + sound.settings.fadeInMs / 1000
        );
      }

      // Connect: source -> gain -> effects chain -> destination
      source.connect(gainNode);
      this.connectEffectsChain(gainNode);

      // Create a dummy audio element for compatibility with PlayingSound interface
      const audio = new Audio();
      ...
      // Start playback
      source.start(0);
      console.log(`✅ Sound playing in reverse: ${sound.name}`);

      return playingId;
    } catch (error) {
      console.error('Failed to play sound in reverse:', error);
      throw error;
    }
  }

  // Reverse an audio buffer, respecting start and end markers
  private reverseAudioBuffer(buffer: AudioBuffer, startTime?: number, endTime?: number): AudioBuffer {
    if (!this.audioContext) {
    ...
    // Calculate actual start and end sample indices
    ...
    // Create new buffer for reversed audio (only the segment between markers)
